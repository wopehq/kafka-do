...
// Returns the messages when as much as batchSize is arrived or time exceed,
// (if there is no message on any priority stage, time will may be exceed.)
func ConsumeBatchPriority(ctx context.Context, client sarama.ConsumerGroup, topics []string, batchSize int) ([]sarama.ConsumerMessage, string) {
	if batchSize == 0 {
		return nil, ""
	}
